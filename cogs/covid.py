...
class covid(commands.Cog):
    ...
    try:
      ...
      world = Request('https://www.worldometers.info/coronavirus/', headers=header)
      world = re.sub(r'<.*?>', lambda g: g.group(0).upper(), urlopen(world).read().decode('utf-8') )

      US = Request('https://www.worldometers.info/coronavirus/country/us/', headers=header)
      US = re.sub(r'<.*?>', lambda g: g.group(0).upper(), urlopen(US).read().decode('utf-8') )
    except Exception as inst:
      d = inst
      print(d)
      print(d)

    df_list = pd.read_html(world)
    us_df_list = pd.read_html(US)
...
#type: 'Country,Other', 'TotalCases', 'TotalDeaths', 'NewDeaths', 'TotalRecovered', 'ActiveCases', 'Serious,Critical'
    ...
    #Statistics Command
    @commands.command(name='stat', aliases=['stats', 'statistic', 's', 'cases', 'covid', 'corona', 'coronavirus'])
    ...
    async def stat(self, ctx, location = 'ALL', state = ''):
        ...
        if location == 'ALL' or (location in list(alpha2.values())) :
            ...
            if state:
                ...
                description='React with 📈 for a **linear** graph or 📉 for a **log** graph'
            embed = discord.Embed(
                description=description,
                colour=discord.Colour.red(),
                timestamp=datetime.utcnow()
                )
            embed.add_field(name=':thermometer_face: Confirmed', value= f'**{int(confirmed)}** {new_confirmed}')
            embed.add_field(name=':skull: Deaths', value=f'**{int(deaths)}** {new_deaths}')
            if state:
                ...
                embed.add_field(name=':alarm_clock: Active Cases', value=f'**{int(active)}**')
                embed.add_field(name=':100: Mortality Rate', value=f'**{mortality_rate}%**')
                ...
                embed.add_field(name=':grin: Recovered', value=f'**{int(recovered)}**')
                embed.add_field(name=':alarm_clock: Active Cases', value=f'**{int(active)}**')
                embed.add_field(name=':100: Mortality Rate', value=f'**{mortality_rate}%**')
                embed.add_field(name=':neutral_face: Recovery Rate', value=f'**{recovery_rate}%**')
